// cards_example.tsx
// FinApp — Tela de Cartões, em TSX.
//
// Composição: header com título + CTA "Novo", carrossel horizontal com snap
// dos cartões em tamanho grande (chip + wireless + número + titular/validade),
// dots paginadores, painel de detalhes com anel de utilização, estatísticas
// (fatura/parcelas/melhor dia) e lista de transações do cartão ativo.
...
function BigCard({ card, active }: BigCardProps): React.ReactElement {
  return (
    <div style={{
      flex: '0 0 296px',
      ...
      boxShadow: active
        ? '0 18px 40px rgba(21,21,26,0.22), 0 4px 10px rgba(21,21,26,0.08)'
        : '0 6px 14px rgba(21,21,26,0.10)',
    }}>
      ...
      <div style={{ position: 'relative' }}>
        <div className="finapp-mono" style={{
          fontSize: 17, letterSpacing: '0.12em', opacity: 0.95,
        }}>
          •••• •••• •••• {card.last4}
        </div>
        <div style={{
          ...
          <div>
            <div style={{
              fontSize: 9, opacity: 0.6, letterSpacing: '0.04em', textTransform: 'uppercase',
            }}>Titular</div>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div className="finapp-mono" style={{ fontSize: 11, fontWeight: 500 }}>{card.expiry}</div>
          </div>
        </div>
      </div>
    </div>
...
function UtilizationRing({ pct, color = 'var(--ink)' }: UtilizationRingProps): React.ReactElement {
  const r = 28;
  const c = 2 * Math.PI * r;
  const off = c * (1 - pct);
  return (
...
function Stat({ label, value, sublabel }: StatProps): React.ReactElement {
  return (
    <div style={{ flex: 1 }}>
      <div style={{
...
export default function CardsScreen(): React.ReactElement {
  ...
  const cardTxs = TRANSACTIONS.filter((t: Transaction) => t.card === active.id);
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: '0 22px 18px', fontSize: 13, color: 'var(--muted)' }}>
          {CARDS.length} cartões · {CARDS.filter((c: Card) => c.type === 'credit').length} de crédito
        </div>

        {/* Carousel */}
        <div
          ref={scrollRef}
          className="finapp-noscroll"
          style={{
        ...
        {/* Dots */}
        <div style={{
          display: 'flex', justifyContent: 'center', gap: 6, padding: '14px 0 6px',
        }}>
              ...
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div className="finapp-mono" style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>
        ...
        <div style={{
          padding: '24px 22px 6px', display: 'flex',
        ...
        <div style={{
          margin: '0 16px 0', background: 'var(--surface)', borderRadius: 18,
          padding: '4px 16px', border: '1px solid var(--hairline)',
        }}>
          {cardTxs.length === 0 ? (
            <div style={{
              padding: '22px 4px', textAlign: 'center', color: 'var(--muted)', fontSize: 13,
            }}>
              Nenhuma transação neste cartão ainda.
            </div>
          ) : cardTxs.map((tx: Transaction, i: number) => (
